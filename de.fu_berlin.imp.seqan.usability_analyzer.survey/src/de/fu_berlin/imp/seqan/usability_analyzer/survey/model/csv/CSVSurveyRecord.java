package de.fu_berlin.imp.seqan.usability_analyzer.survey.model.csv;
...
public class CSVSurveyRecord {
	private Logger logger = Logger.getLogger(CSVSurveyRecord.class);
	...
	public CSVSurveyRecord(String[] keys, String[] values) throws IOException {
		assert keys != null;
		assert values != null;
		...
		if (this.keys.contains(KEY_DATE)) {
			try {
				...
				this.logger.warn("Could not parse date from "
						+ CSVSurveyRecord.class.getSimpleName(), e);
			}
		}
	}

	/**
	 * Returns a list of all keys of this {@link CSVSurveyRecord}.
	 * 
	 * @return
	 */
	...
	public boolean equals(Object obj) {
		...
		if (!(obj instanceof CSVSurveyRecord)) {
			return false;
		}
		CSVSurveyRecord other = (CSVSurveyRecord) obj;
		if (this.keys == null) {
			if (other.keys != null) {
				return false;
			}
