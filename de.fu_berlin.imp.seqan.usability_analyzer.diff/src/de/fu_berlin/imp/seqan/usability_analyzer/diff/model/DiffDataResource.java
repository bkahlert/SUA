...
public class DiffDataResource implements IData, HasDateRange, ICodeable, HasID {

	private static final Logger LOGGER = Logger
			.getLogger(DiffDataResource.class);
	...
	public static ID getId(IData diffFile) {
		ID id = null;
		Matcher matcher = PATTERN.matcher(diffFile.getName());
		if (matcher.find())
	...
	private IData data;
	private DiffDataResource prevDiffFile;
	...
	public DiffDataResource(IData data, DiffDataResource prevDiffFile, ID id,
		...
		this.diffFileRecords = DiffDataResourceUtils.readRecords(this,
				sourceOrigin, sourceCache, progressMonitor);
	}
	...
	public DiffDataResource getPrevDiffFile() {
		return prevDiffFile;
	}
	...
	public int compareTo(DiffDataResource diffDataResource) {
		if (this.getDateRange() == null
				|| this.getDateRange().getStartDate() == null) {
			if (diffDataResource.getDateRange() == null
					|| diffDataResource.getDateRange().getStartDate() == null) {
				return 0;
			} else {
				return -1;
			}
		} else if (diffDataResource.getDateRange() == null
				|| diffDataResource.getDateRange().getStartDate() == null) {
			return 1;
		} else {
			return this.getDateRange().getStartDate()
					.compareTo(diffDataResource.getDateRange().getStartDate());
		}
	}
