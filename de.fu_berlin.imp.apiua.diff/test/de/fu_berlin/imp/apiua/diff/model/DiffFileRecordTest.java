...
public class DiffFileRecordTest {
	...
	public void checkPatches() throws IOException {
		for (ValidationPatch patch : this.validationPatches) {
			Assert.assertEquals(
					"The validation patch's start is not up-to-date!",
					patch.patchStart,
					new String(FileUtils.readBytesFromTo(
							this.underlayingDiffFile, patch.start, patch.start
									+ patch.patchStart.length())));
			Assert.assertEquals(
					"The validation patch's end is not up-to-date!",
					patch.patchEnd,
					new String(FileUtils.readBytesFromTo(
							this.underlayingDiffFile, patch.end
									- patch.patchEnd.length(), patch.end)));
		}
	}
	...
	public void testGetPatch() throws URISyntaxException, IOException {
		...
		final Diff diff = new Diff(data, null, trunk, sourceCache,
			...
			public List<String> getContent(long contentStart, long contentEnd) {
				if (this.i < DiffFileRecordTest.this.validationPatches.size()) {
					ValidationPatch patch = DiffFileRecordTest.this.validationPatches
							.get(this.i);
