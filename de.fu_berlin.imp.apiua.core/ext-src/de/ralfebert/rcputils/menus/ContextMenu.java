...
public abstract class ContextMenu {
	 ...
	 */
	public ContextMenu(Control control, final IWorkbenchPartSite site) {
		this.viewer = null;
		this.site = site;
		...
		if (control instanceof ISelectionProvider) {
			site.setSelectionProvider((ISelectionProvider) control);
		}
	...
	public ContextMenu(Viewer viewer, final IWorkbenchPartSite site) {
		this(viewer != null ? viewer.getControl() : null, site);
		if (viewer != null) {
			this.viewer = viewer;
	...
	private final IDoubleClickListener listenerExecuteDefaultCommand = event -> {
		...
		try {
			String defaultCommandID = ContextMenu.this.getDefaultCommandID();
